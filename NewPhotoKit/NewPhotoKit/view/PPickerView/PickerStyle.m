...
- (UIFont *)titleTextFont {
    if (!_titleTextFont) {
        _titleTextFont = [UIFont systemFontOfSize:15.f];
    }
...
#pragma  mark -坦框模式樣板1 - 取消/確定按鈕圓角樣式
+ (instancetype)pickerStyleWithThemeColor:(UIColor *)themeColor {
    PickerStyle *customStyle = [[self alloc]init];
    if (themeColor) {
        customStyle.cancelTextColor = themeColor;
        customStyle.cancelBorderStyle = BorderStyleSolid;
        customStyle.doneColor = themeColor;
        customStyle.doneTextColor = [UIColor whiteColor];
        customStyle.doneBorderStyle = BorderStyleFill;
    }
    return customStyle;
}

#pragma mark -弹框模版样式2 -顶部圆角样式 + 完成按钮样式
+ (instancetype)pickerStyleWithDoneTextColor:(UIColor *)doneTextColor {
    ...
    if (doneTextColor) {
        ...
        customStyle.doneTextFont = [UIFont boldSystemFontOfSize:16.f];
...
#pragma mark 弹框模版样式3 - 顶部圆角样式 + 图标按钮
